...
def generate():
    ...
    np.savez_compressed(os.path.join(base_dir, 'data_train/word2index.npz'),
    ...
    for subset, enfile, zhfile in zip(subsets, enfiles, zhfiles):
        ens = [];zhs = []
        with open(data_dir + enfile, 'r', encoding='utf-8') as fen, \
                open(data_dir + zhfile, 'r', encoding='utf-8') as fzh:
        ...
        save2file = os.path.join(base_dir, 'data_{}/{}.npz'.format(subset, subset))
        np.savez_compressed(save2file, ens=ens, zhs=zhs)

def getdata(subset):
    file = os.path.join(base_dir, 'data_{}/{}.npz'.format(subset, subset))
    if not os.path.exists(file):
        generate()

    data = np.load(file)
    word2index = np.load(os.path.join(base_dir, 'data_train/word2index.npz'))
