...
def eval(opt, model, best_score, checkpoint_id, epoch, batch):
    ...
    for sentence_en, sentence_zh in dataloader:
        ...
        _, loss = model(sentence_en, sentence_zh, target_len=opt.max_len,
                                  label_smooth=opt.label_smooth, dropout=opt.dropout)
...
activate
cd $HOME/gyzhu/nmt2 
python main_wmt17.py train --ngpu=7 --eval_iter=500 --lr=0.001 --beam-size=0 \
