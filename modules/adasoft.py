...
class AdaptiveLoss(nn.Module):
    def __init__(self, cutoff, label_smooth):
        ...
        for _ in self.cutoff:
            if self.label_smooth > 0:
                ...
                self.criterions.append(nn.NLLLoss(size_average=False, ignore_index=Constants.PAD_INDEX))
    ...
    def adjust_target_label_smooth(self, inputs, targets):
        label_smooth = [self.label_smooth, self.label_smooth, self.label_smooth]
        ...
        for i in range(len(targets)):
            ...
            if i == 0:
                ...
                true_dist = inputs[i].data.clone()
                true_dist.fill_(label_smooth[i] / (len(true_dist[0]) - 1))
                true_dist.scatter_(1, targets[i].data.unsqueeze(1), 1 - label_smooth[i])
            targets[i] = true_dist
        return targets
